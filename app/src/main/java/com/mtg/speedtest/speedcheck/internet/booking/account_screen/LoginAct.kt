    ...
    private fun openMainAct() {
        ...
        if (email.isEmpty() || password.isEmpty()) {
            ...
            for (i in 0 until userList.size) {
                if (email == userList[i].emailUser && password == userList[i].passwordUser) {
                    PreferManager.getInstance(this)?.write(CommonUtils.keyIndexAccount, i)
                    startActivity(Intent(this, MainAct::class.java))
                    return
                }
            }
            Toast.makeText(this, resources.getString(R.string.account_error), Toast.LENGTH_LONG)
                .show()
        }
    }
